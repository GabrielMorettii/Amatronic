...
describe('Update Sales Controller', ()=>{
  beforeAll(async ()=>{
  ...
  it('Should be able to update a sale', async ()=>{
    ...
    const {id} = responseSale.body;

    const response = await request(app).put(`/sales/${id}`).send({
      quantity: 5,
    }).set({
      Authorization: `Bearer ${tokenAuth}`
    })

    expect(response.body.totalValue).toEqual(5001.6);
    expect(response.status).toBe(200);
  })

  it('Should not be able to update a non-existent sale', async ()=>{
    const response = await request(app).put(`/sales/${uuidv4()}`).send({
      quantity: 5,
    }).set({
      Authorization: `Bearer ${tokenAuth}`
    })
